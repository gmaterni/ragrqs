...
return this._createResult(!1,null,null,g)}finally{this.abortController=null}}}
class MistralClient extends LLMClient{constructor(a){super(a,"https://api.mistral.ai/v1/chat/completions")}async sendRequest(a,b=60){a=await this._fetch(this.baseUrl,a,{"Content-Type":"application/json",Authorization:`Bearer ${this.apiKey}`},b);if(a.ok)try{return this._createResult(!0,a.response,a.response.choices[0].message.content)}catch(c){return this._createResult(!1,null,null,this._createError("Invalid response structure","ParsingError",null,c))}else return a}}
...
"ParsingError",null,d))}else return b}_convertPayloadToGemini(a){return{contents:a.messages.map(b=>({role:"system"===b.role?"user":b.role,parts:[{text:b.content}]})),generationConfig:{temperature:a.temperature,maxOutputTokens:a.max_tokens}}}}function umgm(a){return a.map(b=>atob(b).split("").map(c=>String.fromCharCode((c.charCodeAt(0)-5+256)%256)).join("")).join("")}
function getLlmClient(a){console.log("** MODEL:\n",a);if(a.toLowerCase().includes("gemini"))return a=umgm(["Rk5/Zlh+RlI=","Z29ROnlnW1A=","VXlTXHxWSnc=","PXR/VntSZG8=","e1hwU0hPaA=="]),new GeminiClient(a);a=umgm(["S0ZaeFJ4W0s=","WHw6bFw6VEo=","cHtaX0pfNm8=","aE5cS3FVbzk="]);return new MistralClient(a)};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
