...
this.doc,this.doc=""):(a=this.limitWIthPoint(this.doc,a),this.doc_part=this.doc.substring(0,a),this.doc=this.doc.substring(a).trim())},addPrompt(a){},async requestDocsRAG(a){ThreadMgr.init();DataMgr.deleteDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.ragContext="";this.responses=[];this.prompts=[];var b=0;try{const c=promptDoc(",").length;for(let d=0;d<DataMgr.docs.length;d++){const e=DataMgr.docs[d];if(""==e.trim())continue;const f=DataMgr.doc_names[d];++b;this.doc=e;
const h=this.doc.length;UaLog.log(`${f} (${h}) `);xlog(`${f} (${h}) `);let l=0;for(;;){l++;this.setDocPart(c);if(10>this.doc_part.length)break;let g=promptDoc(this.doc_part,a);const k=this.checkInput(g);if(0>k){UaLog.log(`RAG error size: ${k}`);const n=this.doc_part.substring(0,this.doc_part.length+k);g=promptDoc(n,a)}this.log(`${b},${l}`);const p=getPayloadDoc(g);let m=await requestPost(p);m=`# titolo:${f} parte:${l}. \n${m}`;this.responses.push(m)}}}catch(c){alert("requestDocsRAG(1)\n"+c),xerror(c)}try{const c=
list2text(this.responses);let d=promptToContext(c,a);const e=this.checkInput(d);if(0>e){UaLog.log(`Responses error size: ${e}`);const h=accumaltes.substring(0,accumaltes.length+e);d=promptToContext(h,a)}UaLog.log(`Contesto  (${d.length})`);const f=getPlayloadContext(d);this.ragContext=await requestPost(f);this.saveToDb()}catch(c){throw xerror(c),alert("requestDocsRAG(2)\n"+c),c;}b="";try{let c=promptWithContext(this.ragContext,a);const d=this.checkInput(c);if(0>d){UaLog.log(`ToContext error size: ${d}`);
...
0>f&&(UaLog.log(`Conv. error size: ${f}`),d=d.substring(-f),e=promptThread(b,d,a));this.addPrompt(e);UaLog.log(` Conv.  (${e.length})`);const h=getPayloadQuery(e);let l=await requestPost(h);ThreadMgr.add(a,l);c=ThreadMgr.getOutText()}catch(d){throw alert("requestContext(2) \n"+d),c=d,xerror(d),d;}finally{return c}}}},ThreadMgr={rows:[],init(){this.rows=[]},clean(a){return a=a.replace(/\n\s*\n/g,"\n")},add(a,b){b=this.clean(b);this.rows.push([a,b])},getOutText(){const a=[];for(const b of this.rows){const c=
b[0].trim(),d=b[1].trim();a.push(`<user>:\n${c}\n<Assistant>:\n${d}\n\n`)}return a.join("").trim().replace(/#+\s*(\w+)\s*(:\s*)?/,"")},getThread(){const a=[];for(const b of this.rows){const c=b[0].trim(),d=b[1].trim();a.push(`<user>:\n${c}\n<Assitant>:\n${d}\n\n`)}return a.join("").trim()},isFirst(){return 0==this.rows.length}};const VERS="v 0.1.26";var xlog=console.log,xerror=console.error;let T0={};var start_performance=function(a){T0.k=performance.now()},log_performance=function(a,b=""){a=performance.now();const c=a-T0.k;console.log(b,c);T0.k=a;return c};function openApp(){wnds.init();Menu.init();TextInput.init();TextOutput.init();document.querySelector(".menu-btn").checked=!1;release()}
