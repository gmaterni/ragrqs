...
`}function promptThread(a,b){return`
  ${a}
  ${b}
`}
...
async function requestGet(a){try{var b=await fetch(a,{method:"GET",headers:{"Content-Type":"text/plain;charset=UTF-8"}});if(!b.ok)throw Error(`HTTP error! status: ${b.status}`);const c=await b.arrayBuffer();return(new TextDecoder("utf-8")).decode(c)}catch(c){throw console.error(c),b="AbortError"===c.name?"Request was aborted":c.message.includes("HTTP error! status")?c.message:"An error occurred",alert(`requestGet()\nurl: ${a}\n${b}`),c;}};const MAX_PROMPT_LENGTH=91136,Rag={ragContext:"",ragQuery:"",responses:[],prmpts:[],doc:"",doc_part:"",responseText:"",num_rsp:0,saveToDb(){UaDb.saveJson("id_rag",{context:this.ragContext,ragquery:this.ragQuery})},readFromDb(){const a=UaDb.readJson("id_rag");this.ragContext=a.context||"";this.ragQuery=a.ragquery||""},saveRespToDb(){UaDb.saveArray("id_responses",this.responses)},readRespsFromDb(){this.responses=UaDb.readArray("id_responses")},responsesLength(){return this.responses.reduce((a,b)=>a+
b.length,0)},log(a){const b=MAX_PROMPT_LENGTH,c=this.doc.length,d=this.doc_part.length,e=this.responsesLength();xlog(`${a} mx:${b} d.rest:${c} d.part:${d} rsps:${e}`);UaLog.log(`${a}${"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"}${c}${"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"}${d}${"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"}${e}`)},checkInput(a){return MAX_PROMPT_LENGTH-a.length},setDocPart(){var a=MAX_PROMPT_LENGTH;if(this.doc.length<a)this.doc_part=this.doc,this.doc="";else{const b=this.doc.indexOf(".",a);a=
(-1!=b?b:a)+1;this.doc_part=this.doc.substring(0,a);this.doc=this.doc.substring(a).trim()}},async requestDocsRAG(a){this.num_rsp=0;ThreadMgr.init();DataMgr.deleteDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.responseText=this.ragContext="";this.responses=[];this.prompts=[];let b=0;try{for(let c=0;c<DataMgr.docs.length;c++){const d=DataMgr.docs[c];if(""==d.trim())continue;const e=DataMgr.doc_names[c];++b;this.doc=d;const f=this.doc.length;UaLog.log(`${e} (${f}) `);xlog(`${e} (${f}) `);
let h=0;for(;;){h++;this.setDocPart();if(10>this.doc_part.length)break;let l=promptDoc(this.doc_part,a);const g=this.checkInput(l);if(0>g){UaLog.log(`RAG error size: ${g}`);const n=this.doc_part.substring(0,this.doc_part.length+g);l=promptDoc(n,a)}this.num_rsp++;this.log(`${b},${h}`);const k=getPayloadDoc(l);let m=await requestPost(k);m=cleanResponse(m);m=`# titolo:${e} parte:${h}. \n${m}`;this.responses.push(m)}}}catch(c){alert("requestDocsRAG(1)\n"+c),xerror(c)}try{const c=list2text(this.responses);
let d=promptToContext(c,a);const e=this.checkInput(d);if(0>e){UaLog.log(`Responses error size: ${e}`);const h=accumaltes.substring(0,accumaltes.length+e);d=promptToContext(h,a)}this.prompts.push(d);this.num_rsp++;UaLog.log(`Contesto  (${d.length})`);const f=getPlayloadContext(d);this.ragContext=await requestPost(f);this.saveToDb()}catch(c){throw xerror(c),alert("requestDocsRAG(2)\n"+c),c;}try{let c=promptWithContext(this.ragContext,a);this.prmpts.push(c);this.num_rsp++;UaLog.log(`Risposta  (${c.length})`);
const d=getPayloadQuery(c);this.responseText=await requestPost(d);this.responseText=cleanResponse(this.responseText);this.responses.push(this.responseText);this.saveRespToDb();this.saveToDb()}catch(c){throw this.responseText=c,xerror(c),alert("requestDocsRAG(3)\n"+c),c;}finally{return this.responseText}},async requestContext(a){this.readFromDb();if(!this.ragContext&&!confirm("Contesto vuoto, continuare?"))return"";if(ThreadMgr.isFirst())try{let b=this.ragContext,c=promptWithContext(b,a);const d=this.checkInput(c);
0>d&&(UaLog.log(`Conv. error size: ${d}`),b=this.ragContext.substring(-d),c=promptWithContext(b,a));ThreadMgr.add("",b);UaLog.log(`Inizio Conv.  (${c.length})`);const e=getPayloadQuery(c);this.responseText=await requestPost(e);this.responseText=cleanResponse(this.responseText);ThreadMgr.add(a,this.responseText);this.responseText=ThreadMgr.getText()}catch(b){throw this.responseText=b,xerror(b),alert("requestContext(1) \n"+b),b;}finally{return this.responseText}else try{let b=ThreadMgr.getText(),c=
promptThread(b,a);const d=this.checkInput(c);0>d&&(UaLog.log(`Conv. error size: ${d}`),b=b.substring(-d),c=promptThread(b,a));UaLog.log(` Conv.  (${c.length})`);const e=getPayloadQuery(c);this.responseText=await requestPost(e);this.responseText=cleanResponse(this.responseText);ThreadMgr.add(a,this.responseText);this.responseText=ThreadMgr.getText()}catch(b){throw this.responseText=erroor,xerror(b),alert("requestContext(2) \n"+b),b;}finally{return this.responseText}}},ThreadMgr={rows:[],init(){this.rows=
[]},clean(a){return a.replace(/<[^>]+>\s*:?/g,"")},add(a,b){b=this.clean(b);this.rows.push([a,b])},getRows(){return this.rows},getText(){const a=[];for(const b of this.rows){const c=b[0].trim(),d=b[1].trim();""==c?a.push(`<Contesto>:\n${d}\n\n`):a.push(`<User>:\n${c}\n<LLM>:\n${d}\n\n`)}return a.join("").trim()},isFirst(){return 0==this.rows.length}};const VERS="v 0.1.24";var xlog=console.log,xerror=console.error;let T0={};var start_performance=function(a){T0.k=performance.now()},log_performance=function(a,b=""){a=performance.now();const c=a-T0.k;console.log(b,c);T0.k=a;return c};function openApp(){wnds.init();Menu.init();TextInput.init();TextOutput.init();document.querySelector(".menu-btn").checked=!1;release()}
...
let focusFlag=!1;
const TextInput={wnd:null,init(){this.inp=document.querySelector(".text-input");document.addEventListener("keydown",a=>{document.activeElement!==this.inp&&a.preventDefault()});document.querySelector(".send-input").addEventListener("click",()=>this.send());document.querySelector(".send2-input").addEventListener("click",()=>this.send2());document.querySelector(".clear-input").addEventListener("click",()=>this.clear())},handleEnter(a){},async send(){showSpinner();setOutText("");var a=this.inp.value.trim();
try{const b=await Rag.requestDocsRAG(a);setOutText(b);this.inp.value="";UaLog.close()}catch(b){a=`send\n${b}`,xerror(a),alert(a),setOutText(a)}hideSpinner()},async send2(){showSpinner();ThreadMgr.isFirst()&&setOutText("");var a=this.inp.value.trim();try{const b=await Rag.requestContext(a);if(""==b){hideSpinner();return}setOutText(b);this.inp.value=""}catch(b){a=`send2\n${b}`,xerror(a),alert(a),setOutText(a)}hideSpinner()},clear(){confirm("Confermi cancellazione conversazione? ")&&(this.inp.value=
"",setOutText(""),ThreadMgr.init())}},setOutText=a=>{var b=document.getElementById("id-text-out");b.innerHTML='<pre class="pre-text"></pre>';b=b.querySelector(".pre-text");b.textContent=a;b.scrollTop=b.scrollHeight};
