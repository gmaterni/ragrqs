...
function elencoDati(a){var b=UaDb.getAllIds();a=[];for(var c of b)b=UaDb.read(c).length,a.push(`${c} (${b})`);c=a.join("\n ");wnds.wpre.show(c)}const showT=a=>{wnds.wpre.show(DataMgr.docs[a])};function elencoDocs(){DataMgr.readDbDocs();DataMgr.readDbDocNames();var a=DataMgr.doc_names,b=UaJtfh();let c=0;b.append("<ul>");for(const k of a){a=b;var d=a.append,e=k,g=c++;d.call(a,`
      <li><a href="#" onclick="showT(${g});">${g+1}.${e}</a></li>
  `)}b.append("</ul>");b=b.html();wnds.wdiv.show(`<br><br>${b}`)}function calcQuery(){DataMgr.readDbDocs();DataMgr.readDbDocNames();var a=[];let b=0,c=0;a.push("Documento   Num.Parti");a.push("==================");for(const d of DataMgr.docs){const e=DataMgr.doc_names[c];c+=1;const g=Math.ceil(d.length/MAX_PROMPT_LENGTH);b+=g;a.push(`${e}&nbsp;&nbsp;&nbsp;[${g}]`)}a.push("==================");a.push(`Totale num. Parti: ${b}`);a=a.join("\n");wnds.wpre.show(a)}
...
a;ThreadMgr.initThread();this.saveToDb();var b=0;try{let e=1;for(let g=0;g<DataMgr.docs.length;g++){let k=DataMgr.docs[g];if(""==k.trim())continue;const h=DataMgr.doc_names[g],f=k.length;xlog(`${h} (${f}) `);UaLog.log(`${h} (${f}) `);++b;var c=1;let l=0,m="",n="",q="",v=[];for(;;){const u=getPartSize(k,promptDoc("",a,""),l);if(10>u)break;[m,n]=getPartDoc(k,u);ragLog(`${e}) ${b},${c}`,m.length,n.length,this.answers);const w=promptDoc(m,this.ragquery),t=getPayloadDoc(w),p=await getResponse(t,90);if(!p)return"";
const r=p.error;if(!p.ok){console.error("ERR1\n",r);const y=r.code;if(400==y)if(isTooLarge(r)){UaLog.log(`Error tokens Doc ${prompt.length}`);l+=PROMPT_DECR;continue}else throw r;else if(408==y){UaLog.log("Error timeout Context");continue}else throw r;}q=p.data;if(!q)return"";const z=p.response;showTokens(z);calcTokens.add(z);c++;e++;k=n;q=cleanResponse(q);v.push(q);this.answers.push(`DOCUMENTO : ${h}_${c}\n${q}`)}const A=v.length;let x=v.join("\n\n");for(c="";;){const u=promptBuildContext(x,this.ragQuery),
w=getPayloadBuildContext(u),t=await getResponse(w,90);if(!t)return"";const p=t.error;if(!t.ok){console.error("ERR2\n",p);const r=p.code;if(400==r)if(isTooLarge(p)){UaLog.log(`Error tokens build Context ${prompt.length}`);x=truncateInput(x,PROMPT_DECR);continue}else throw p;else if(408==r){UaLog.log("Error timeout Context");continue}else throw p;}c=t.data;if(!c)return"";calcTokens.add(t.response);break}UaLog.log(`context  ${A} => ${c.length}`);c=`\n### DOCUMENTO: ${h}\n ${c}`;this.docContextLst.push(c)}}catch(e){throw console.error("ERR3\n",
e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();a="";b=this.ragContext;try{for(;;){const g=promptWithContext(b,this.ragQuery),k=getPayloadWithContext(g),h=await getResponse(k,90);if(!h)return"";const f=h.error;if(!h.ok){console.error("ERR4\n",f);const l=f.code;if(400==l)if(isTooLarge(f)){UaLog.log(`Error tokens with Context ${g.length}`);b=truncateInput(b,PROMPT_DECR);continue}else throw f;else if(408==l){UaLog.log("Error timeout Context");continue}else throw f;}a=h.data;if(!a)return"";
calcTokens.add(h.response);break}this.ragAnswer=a=cleanResponse(a);this.saveRespToDb();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);var d=calcTokens.get_sum_input_tokens();const e=calcTokens.get_sum_generate_tokens();UaLog.log(`Tokens: ${d} ${e}`);d=[];d.push({role:"user",content:this.ragQuery});d.push({role:"assistant",content:a});return messages2html(d)}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";ThreadMgr.isFirst()?(console.log("******** FIRST"),
this.ragContext?(a=promptThreadRag(this.ragContext,this.ragQuery,this.ragAnswer,a),ThreadMgr.addMessages(a)):(a=promptThread(a),ThreadMgr.addMessages(a))):(console.log("******** NEXT"),ThreadMgr.addQuery(a));try{const c=ThreadMgr.getMessages(),d=getPayloadThread(c);for(;;){const k=await getResponse(d,90);if(!k)return"";const h=k.error;if(!k.ok){console.error("ERR6\n",h);const l=h.code;if(400==l)throw isTooLarge(h)&&alert("Conversazione troppo lunga"),h;if(408==l)continue;else throw h;}b=k.data;if(!b)return"";
const f=k.response;showTokens(f);calcTokens.add(f);break}b=cleanResponse(b);ThreadMgr.addAnswer(b);const e=ThreadMgr.getUserMessages(),g=messages2html(e);UaLog.log(`Inizio Conversazione (${prompt.length})`);return g}catch(c){throw console.error("ERR7\n",c),c;}}},ThreadMgr={rows:[],initThread(){this.rows=[]},delete(){alert("DELETE");this.rows=[]},addMessages(a){for(const b of a)this.rows.push(b);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getMessages(){return this.rows},getUserMessages(){return this.rows.filter((a,
b)=>"system"!==a.role)},addQuery(a){this.addMessages([{role:"user",content:a}])},addAnswer(a){this.addMessages([{role:"assistant",content:a}])},isFirst(){return 1>this.rows.length}};function getPayloadDoc(a){return{model:"",temperature:.3,max_tokens:1024,stream:!1,random_seed:42,messages:a,safe_prompt:!1}}function getPayloadBuildContext(a){return{model:"",messages:a,temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadWithContext(a){return{model:"",messages:a,temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}
function getPayloadThread(a){return{model:"",messages:a,temperature:.7,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}};function promptDoc(a,b){return[{role:"system",content:"Estrattore semantico di informazioni ottimizzato per elaborazione da un LLM.\n\n# OBIETTIVO\nEstrarre e condensare le informazioni essenziali dal TESTO per rispondere alla DOMANDA, in formato ottimizzato per elaborazione computazionale.\n\n# ISTRUZIONI\n1. Analizza il testo e seleziona SOLO le informazioni direttamente rilevanti per la domanda.\n2. Elimina contenuti irrilevanti o ridondanti.\n3. Mantieni relazioni logiche essenziali (causali, temporali, concettuali).\n4. Preserva terminologia specialistica necessaria.\n5. Produci proposizioni atomiche semanticamente complete.\n6. Usa sintassi minimale (soggetto-verbo-complemento).\n7. Elimina elementi discorsivi, connettivi e contestualizzazioni non essenziali.\n8. Presenta l'informazione con massima densit\u00e0 semantica, senza ridondanze.\n\n# FORMATO\n- Struttura flat senza gerarchie\n- Non numerare le proposizioni.\n- Non usare introduzioni o conclusioni.\n- Non includere spiegazioni meta-testuali.\n- Non inserire frasi di collegamento tra le proposizioni.\n"},
...
var UaDrag=function(a){return function(b){let c=0,d=0,e=0,g=0;const k=function(f){f=f||window.event;f.preventDefault();c=e-f.clientX;d=g-f.clientY;e=f.clientX;g=f.clientY;b.style.top=b.offsetTop-d+"px";b.style.left=b.offsetLeft-c+"px"},h=function(){document.onmouseup=null;document.onmousemove=null};b.onmousedown=function(f){f=f||window.event;let l=f.target;!(l=l||null)||nodrag_tds.includes(l.tagName.toLowerCase())||l.classList.contains(nodrag_cls)||(f.preventDefault(),e=f.clientX,g=f.clientY,document.onmouseup=
h,document.onmousemove=k)}}(a)};const UaJtfh=()=>({rows:[],init(){this.rows=[];return this},insert(a){this.rows.unshift(a);return this},append(a){this.rows.push(a);return this},text(a=""){return this.rows.join(a)},html(a=""){return this.rows.join(a).replace(/\s+|\[rn\]/g," ")}});function formatRow(a,b){return a.map((c,d)=>{d=b[d];return 0>d?c.toString().padStart(Math.abs(d)," "):c.toString().padEnd(d," ")}).join(" ")}
