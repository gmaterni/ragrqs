...
"ParsingError",null,d))}else return b}_convertPayloadToGemini(a){return{contents:a.messages.map(b=>({role:"system"===b.role?"user":b.role,parts:[{text:b.content}]})),generationConfig:{temperature:a.temperature,maxOutputTokens:a.max_tokens}}}}function getLlmClient(a){console.log("** MODEL:\n",a);return a.toLowerCase().includes("gemini")?new GeminiClient("AIzaSyAMbjL5tbVKPtNWwQEr8ozQvM_jvSkNCJc"):new MistralClient("FAUsMsVFSw5gW5OEkvUZEZ1jcIWFlPj4")};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
