window.GeminiAdapter={buildUrl:(a,b)=>`${"https://generativelanguage.googleapis.com/v1beta/models/"}${a}:generateContent?key=${b}`,buildHeaders:()=>({"Content-Type":"application/json"}),adaptPayload:a=>{const b={HARM_CATEGORY_HARASSMENT:"HARM_CATEGORY_HARASSMENT",HARM_CATEGORY_HATE_SPEECH:"HARM_CATEGORY_HATE_SPEECH",HARM_CATEGORY_SEXUALLY_EXPLICIT:"HARM_CATEGORY_SEXUALLY_EXPLICIT",HARM_CATEGORY_DANGEROUS_CONTENT:"HARM_CATEGORY_DANGEROUS_CONTENT"},c={};var d=a.messages||[];const e=[];let f=null;for(const g of d)"system"===
g.role?f=g.content:e.push({role:"assistant"===g.role?"model":"user",parts:[{text:g.content}]});c.contents=e;f&&(c.system_instruction={parts:[{text:f}]});d={maxOutputTokens:a.max_tokens,temperature:a.temperature,topP:a.top_p,topK:a.top_k,candidateCount:1,stopSequences:a.stop||[]};for(const g in d)void 0!==d[g]&&null!==d[g]||delete d[g];c.generationConfig=d;!1===a.safe_prompt&&(c.safetySettings=Object.values(b).map(g=>({category:g,threshold:"BLOCK_NONE"})));return c},parseResponse:a=>{try{return a.candidates[0].content.parts[0].text}catch(b){return null}}};window.GroqAdapter={buildUrl:()=>"https://api.groq.com/openai/v1/chat/completions",buildHeaders:a=>({"Content-Type":"application/json",Authorization:`Bearer ${a}`}),adaptPayload:a=>{a={...a};delete a.safe_prompt;delete a.random_seed;return a},parseResponse:a=>{try{return a.choices[0].message.content}catch(b){return null}}};class HttpLlmClient{constructor(a,b){this.adapter=a;this.apiKey=b;this.abortController=null;this.isCancelled=!1;if(!this.adapter)throw Error("Un'istanza di HttpLlmClient richiede un adattatore valido.");if(!this.apiKey)throw Error("Un'istanza di HttpLlmClient richiede una API key valida.");}async sendRequest(a,b=60){var c=a.model;if(!c)return this._createResult(!1,null,null,this._createError("Il campo 'model' \u00e8 obbligatorio nel payload.","ValidationError"));c=this.adapter.buildUrl(c,this.apiKey);
const d=this.adapter.buildHeaders(this.apiKey);a=this.adapter.adaptPayload(a);b=await this._fetch(c,a,d,b);if(b.ok)try{const e=this.adapter.parseResponse(b.response);if(null===e)throw Error("Impossibile estrarre i dati dalla risposta.");return this._createResult(!0,b.response,e)}catch(e){return this._createResult(!1,b.response,null,this._createError("Struttura della risposta non valida.","ParsingError",null,e))}else return this.adapter.parseError&&(b.error=this.adapter.parseError(b.error)),b}cancelRequest(){this.isCancelled=
!0;return this.abortController?(this.abortController.abort(),this.abortController=null,!0):!1}_createResult(a,b=null,c=null,d=null){return{ok:a,response:b,data:c,error:d}}_createError(a,b,c,d){return{message:a||null,type:b||null,code:c||null,details:d||null}}async _handleHttpError(a){const b={400:"Richiesta non valida (Bad Request)",401:"Non autorizzato (Unauthorized) - Controlla la API key",403:"Accesso negato (Forbidden)",404:"Endpoint non trovato (Not Found)",429:"Troppe richieste (Too Many Requests) - Rate limit superato",
500:"Errore interno del server (Internal Server Error)",503:"Servizio non disponibile (Service Unavailable)"}[a.status]||`Errore HTTP ${a.status}`;let c;try{c=await a.json()}catch(d){c=await a.text()}return this._createError(b,"HTTPError",a.status,c)}_handleNetworkError(a){return"AbortError"===a.name?this._createError(this.isCancelled?"Richiesta annullata dall'utente":"Richiesta interrotta per timeout",this.isCancelled?"CancellationError":"TimeoutError",this.isCancelled?499:408):a instanceof TypeError?
this._createError("Errore di rete. Controlla la connessione o l'URL dell'endpoint.","NetworkError"):this._createError("Errore di rete imprevisto.","UnknownNetworkError",null,a)}async _fetch(a,b,c,d){this.isCancelled=!1;this.abortController=new AbortController;const {signal:e}=this.abortController;d=setTimeout(()=>this.abortController.abort(),1E3*d);try{const f=await fetch(a,{method:"POST",headers:c,body:JSON.stringify(b),signal:e});clearTimeout(d);if(this.isCancelled)return this._createResult(!1,
null,null,this._handleNetworkError(new DOMException("Request aborted","AbortError")));if(!f.ok){const k=await this._handleHttpError(f);return this._createResult(!1,null,null,k)}const g=await f.json();return this._createResult(!0,g)}catch(f){clearTimeout(d);const g=this._handleNetworkError(f);return this._createResult(!1,null,null,g)}finally{this.abortController=null}}}window.HttpLlmClient=HttpLlmClient;window.HuggingFaceAdapter={buildUrl:a=>"https://router.huggingface.co/v1/chat/completions",buildHeaders:a=>({"Content-Type":"application/json",Authorization:`Bearer ${a}`}),adaptPayload:a=>{a={model:a.model,messages:a.messages,temperature:a.temperature,max_tokens:a.max_tokens,top_p:a.top_p,top_k:a.top_k,stop:a.stop};for(const b in a)void 0===a[b]&&delete a[b];return a},parseResponse:a=>{try{return a.choices[0].message.content}catch(b){return null}}};window.MistralAdapter={buildUrl:()=>"https://api.mistral.ai/v1/chat/completions",buildHeaders:a=>({"Content-Type":"application/json",Authorization:`Bearer ${a}`}),adaptPayload:a=>{a={...a};delete a.safe_prompt;return a},parseResponse:a=>{try{return a.choices[0].message.content}catch(b){return null}}};const VERS="M 0.2.3";var xlog=console.log;const stopRequest=async()=>{await confirm("Confermi Cancellazione Richeista ?")&&(cancelClientRequest(),hideSpinner())},showSpinner=()=>{const a=document.getElementById("spinner");a.classList.add("show-spinner");a.addEventListener("click",stopRequest)},hideSpinner=()=>{const a=document.getElementById("spinner");a.classList.remove("show-spinner");a.removeEventListener("click",stopRequest)};
...
function messages2html(a){lst=[];for(const c of a){a=c.role.toLowerCase();var b=c.content.replace(/\n{2,}/g,"\n");"assistant"==a?(b=b.replace("## RISPOSTA:","").trim(),b=b.replace(/\./g,".\n").replace(/\n{2,}/g,"\n"),a=`<div class="assistant"><b>Assistant:</b><br>${b}</div>`):"user"==a?(a=b,b=a.indexOf("DOMANDA INIZIALE:"),b=-1!==b?a.slice(b+17).trim():a,a=`<div class="user"><b>User:</b><br>${b}</div>`):"system"==a?a=`<div class="system"><b>System:</b><br>${b}</div>`:(alert("ERROR  in role"),a="<div>ERRROR ROLE</div>");
lst.push(a)}return lst.join("\n")}function messages2text(a){lst=[];for(const b of a){a=b.role.toLowerCase();const c=b.content.trim();"assistant"==a?a=`Assistant:\n${c}\n`:"user"==a?a=`User:\n${c}`:"system"==a?a=`System:\n${c}`:(alert("ERROR  in role"),a="ERRROR ROLE");lst.push(a)}return lst.join("\n")}
function textFormatter(a){a=a.replace(/<[^>]*>/g,"").split(/([.!?:])(?=\s|$)/);let b="";for(let c=0;c<a.length;c+=2){let d=a[c],e=a[c+1]||"";0<d.trim().length&&(b+="  "+d.trim()+e);c<a.length-2&&(b+="\n")}b=b.replace(/User:/g,"\n\nUSER:\n");b=b.replace(/Assistant:/g,"\n\nASSISTANT:\n");return b.trim()};const UaDb={create(a,b){localStorage.getItem(a)?console.error(`ID ${a} already exists.`):localStorage.setItem(a,b)},read(a){a=localStorage.getItem(a);return null===a?"":a},update(a,b){localStorage.getItem(a)&&localStorage.setItem(a,b)},delete(a){localStorage.getItem(a)?localStorage.removeItem(a):console.error(`ID ${a} not found.`)},save(a,b){localStorage.setItem(a,b)},getAllIds(){const a=[];for(let b=0;b<localStorage.length;b++)a.push(localStorage.key(b));return a},saveArray(a,b){b=JSON.stringify(b);
UaDb.save(a,b)},readArray(a){a=UaDb.read(a);return 0==a.trim().length?[]:JSON.parse(a)},saveJson(a,b){b=JSON.stringify(b);UaDb.save(a,b)},readJson(a){return(a=UaDb.read(a))?JSON.parse(a):{}}},DataMgr={docs:[],doc_names:[],linkToName(a){a=a.split("/");return a[a.length-1]},async loadDoc(a){this.readDbDocNames();try{const b=await requestGet(a),c=cleanDoc(b),d=this.linkToName(a);if(this.doc_names.includes(d))alert(`Il documento ${d} \u00e8 gi\u00e0 caricato`);else return this.doc_names.push(d),this.docs.push(c),
this.saveDbDocs(),c}catch(b){alert("loadDoc()\n"+b+"\n"+a)}},addDoc(a,b){b=cleanDoc(b);this.docs.push(b);this.doc_names.push(a);this.saveDbDocs()},saveDbDocs(){UaDb.saveArray(ID_DOC_NAMES,this.doc_names);UaDb.saveArray(ID_DOCS,this.docs)},readDbDocs(){this.docs=UaDb.readArray(ID_DOCS)},readDbDocNames(){return this.doc_names=UaDb.readArray(ID_DOC_NAMES)},deleteJsonDati(){const a=UaDb.getAllIds();for(const b of a)[ID_DOCS,ID_DOC_NAMES].includes(b)||UaDb.delete(b);Rag.ragQuery="";Rag.ragContext="";Rag.ragAnswer=
"";Rag.answers=[];ThreadMgr.rows=[]}};async function requestGet(a){try{var b=await fetch(a,{method:"GET",headers:{"Content-Type":"text/plain;charset=UTF-8"}});if(!b.ok)throw Error(`HTTP error! status: ${b.status}`);const c=await b.arrayBuffer();return(new TextDecoder("utf-8")).decode(c)}catch(c){throw console.error(`Error in requestGet() for url: ${a}`,c),b=c.message.includes("HTTP error! status")?c.message:c.message.includes("NetworkError")?"Network error occurred":"An unknown error occurred",alert(`requestGet()\nurl: ${a}\n${b}`),c;
}}function loadScript(a,b){const c=document.createElement("script");c.src=a;c.onload=()=>{b();document.head.removeChild(c)};c.onerror=()=>{alert(`Errore: Impossibile caricare lo script ${a}`)};document.head.appendChild(c)};const PROVVIDER="mistral",MODEL="mistral-small-latest",adapter=window.MistralAdapter;console.log("*** PROVI;ER: ",PROVVIDER);console.log("*** MODEL: ",MODEL);const apiKey=getApiKey(PROVVIDER),client=new HttpLlmClient(adapter,apiKey),ID_RAG="id_rag",ID_THREAD="id_thread",ID_RESPONSES="id_responses",ID_DOC_NAMES="id_doc_names",ID_DOCS="id_docs",PROMPT_DECR=10240,maxLenRequest=(a=32)=>{a*=3072;return Math.trunc(a+.1*a)},MAX_PROMPT_LENGTH=maxLenRequest(100);
function cancelClientRequest(){client.cancelRequest()}
const isTooLarge=a=>{if(!a)return!1;const b="too large;too long;length exceeded;exceeds maximum length;exceeded;context length".split(";"),c=a.code??a.status;if(413===c)return!0;const d=(e,f)=>{const g=f.map(h=>h.toLowerCase()),k=h=>{if("string"===typeof h){const l=h.toLowerCase();return g.some(q=>l.includes(q))}return Array.isArray(h)?h.some(k):h&&"object"===typeof h?Object.values(h).some(k):!1};return k(e)};a=[a.body,a.message,a.error,a];return 400!==c&&c?!1:a.some(e=>d(e,b))},truncateInput=(a,
b)=>a.substring(0,a.length-b),getPartSize=(a,b,c)=>{c=MAX_PROMPT_LENGTH-c;a.length+b.length<c?c=a.length:(a=a.indexOf(".",c),a=(-1!==a?a:c)+1,a>c+100&&(a=c),c=a);return c},getPartDoc=(a,b)=>{const c=a.substring(0,b);a=a.substring(b).trim();return[c,a]},ragLog=(a,b,c,d)=>{d=d.reduce((e,f)=>e+f.length,0);a=formatRow([a,b,c,d],[8,-7,-7,-7]);UaLog.log(a)},Rag={ragContext:"",ragQuery:"",ragAnswer:"",answers:[],docContextLst:[],init(){ThreadMgr.initThread();this.readRespsFromDb();this.readFromDb()},returnOk(){return 10<
this.ragContext.length},saveToDb(){UaDb.saveJson(ID_RAG,{context:this.ragContext,ragquery:this.ragQuery,raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,this.answers)},readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();
DataMgr.readDbDocNames();DataMgr.readDbDocs();this.docContextLst=[];this.ragQuery=a;ThreadMgr.initThread();this.saveToDb();var b=0;try{let e=1;for(let f=0;f<DataMgr.docs.length;f++){let g=DataMgr.docs[f];if(""==g.trim())continue;const k=DataMgr.doc_names[f];UaLog.log(`${k} (${g.length}) `);++b;var c=1;let h=0,l="",q="",n="",u=[];for(;;){const r=getPartSize(g,promptDoc("",a,""),h);if(10>r)break;[l,q]=getPartDoc(g,r);ragLog(`${e}) ${b},${c}`,l.length,q.length,this.answers);const v=promptDoc(l,this.ragquery),
p=getPayloadDoc(MODEL,v),m=await client.sendRequest(p,90);if(!m)return"";const t=m.error;if(!m.ok){console.error("ERR1\n",t);if(isTooLarge(t)){UaLog.log("Error tokens ");h+=PROMPT_DECR;continue}if(408==t.code){UaLog.log("Error timeout Context");continue}else throw t;}n=m.data;if(!n)return"";c++;e++;g=q;n=cleanResponse(n);u.push(n);this.answers.push(`DOCUMENTO : ${k}_${c}\n${n}`)}const w=u.length;let x=u.join("\n\n");for(c="";;){const r=promptBuildContext(x,this.ragQuery),v=getPayloadBuildContext(MODEL,
r),p=await client.sendRequest(v,90);if(!p)return"";const m=p.error;if(!p.ok){console.error("ERR2\n",m);if(isTooLarge(m)){UaLog.log("Error tokens ");h+=PROMPT_DECR;continue}if(408==m.code){UaLog.log("Error timeout Context");continue}else throw m;}c=p.data;if(!c)return"";break}UaLog.log(`context  ${w} => ${c.length}`);c=`\n### DOCUMENTO: ${k}\n ${c}`;this.docContextLst.push(c)}}catch(e){throw console.error("ERR3\n",e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();a="";b=this.ragContext;
try{for(;;){var d=promptWithContext(b,this.ragQuery);const e=getPayloadWithContext(MODEL,d),f=await client.sendRequest(e,90);if(!f)return"";const g=f.error;if(!f.ok){console.error("ERR4\n",g);if(isTooLarge(g)){UaLog.log("Error tokens}");decr+=PROMPT_DECR;continue}if(408==g.code){UaLog.log("Error timeout Context");continue}else throw g;}a=f.data;if(!a)return"";break}this.ragAnswer=a=cleanResponse(a);this.saveRespToDb();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);d=[];d.push({role:"user",
content:this.ragQuery});d.push({role:"assistant",content:a});return messages2html(d)}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";ThreadMgr.isFirst()?(console.log("*** FIRST"),this.ragContext?(a=promptThreadRag(this.ragContext,this.ragQuery,this.ragAnswer,a),ThreadMgr.addMessages(a)):(a=promptThread(a),ThreadMgr.addMessages(a))):(console.log("*** NEXT"),ThreadMgr.addQuery(a));try{const c=ThreadMgr.getMessages(),d=getPayloadThread(MODEL,c);for(;;){const g=await client.sendRequest(d,
90);if(!g)return"";const k=g.error;if(!g.ok){console.error("ERR6\n",k);if(408==k.code)continue;isTooLarge(k)&&alert("Conversazione troppo lunga");throw k;}b=g.data;if(!b)return"";break}b=cleanResponse(b);ThreadMgr.addAnswer(b);const e=ThreadMgr.getUserMessages(),f=messages2html(e);UaLog.log(`Inizio Conversazione (${prompt.length})`);return f}catch(c){throw console.error("ERR7\n",c),c;}}},ThreadMgr={rows:[],initThread(){this.rows=[]},delete(){alert("DELETE");this.rows=[]},addMessages(a){for(const b of a)this.rows.push(b);
...
*/
const errorDumps=a=>{const b=JSON.stringify(a,null,2);return"{}"==b?`${a}`:b},WndPre=a=>({w:UaWindowAdm.create(a),show(b){wnds.closeAll();b=`
...
setResponseHtml=a=>{const b=document.querySelector("#id-text-out .div-text");b.innerHTML=a;b.style.display="none";b.style.display="";b.scrollTop=b.scrollHeight},TextInput={wnd:null,init(){this.inp=document.querySelector(".text-input");document.addEventListener("keydown",a=>{document.activeElement!==this.inp&&("F1 F2 F3 F4 F5 F6 F7 F8 F9 F10 F11 F12 Control Alt Shift Meta CapsLock Escape PrintScreen ScrollLock Pause Insert Delete Home End PageUp PageDown ArrowLeft ArrowRight ArrowUp ArrowDown".split(" ").includes(a.key)||
